...
On the x axis you have the difference in stimulus strength between one stimulus and the other. For example, when I simulate the same physical speed for both balls, that would be 0 on the x axis. Then on the y axis, we have the proportion of responses “the test stimulus was faster”. These proportions tend to lay on something resembling a Cumulative Gaussian function, which is why we often fit such a function to our data to describe the performance of our participants. When the curve is very steep (which means, the standard deviation of the cumulative Gaussian is very low), that means that the participant is very good at distinguishing the speed of the balls. And the mean of the Cumulative Gaussian would correspond to the strength of the test stimulus the participant perceives as equivalent to the comparison stimulus.
...
First the false positive rates because, well, we can’t even consider methods that gives us inflated false positive rates:
...
Main thing: For the fitting of the GLMMs, Julia isn’t more useful than R. Slightly less, even, because currently it doesn’t give an advantage in fitting durations with regards to R.<br>
<br>
_Some adjacent things:_ <br>

-	Somehow, and I’m not the right person to ask why, the nAGP/fast settings can make a huge difference in Satterthwaite-approximated p values for interactions. So huge that with nAGP = 0/fast = true, we get way more false positives than we should, but agan, only for interactions. Main effects are fine. Does that also translate to more robust ways of assessing significance? I have tried, but I guess someone should.<br>
-	Not treated in this post, but: model specifications can make a huge difference for approximated  p values, with too few random effects often leading to inflated p values for interactions. Again, main effects are mostly fine. I’m currently working on something where we look at that in a bit more detail.<br>
-	Julia is still cute and I’m happy I learned some! <3 <br>